...
export default function EditTestimonialPage() {
  ...
  useEffect(() => {
    fetch("/api/testimonials?all=true")
      ...
      .catch(() => setMessage("Could not load testimonial"))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage(error.message || "Could not update testimonial");
    } finally {
      setSaving(false);
    }
  };
      ...
      <main className="flex-1 overflow-y-auto p-8">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">Update this homepage testimonial.</p>
          </div>
          <button type="button" onClick={() => router.push("/testimonials")} className="admin-btn-secondary">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="admin-btn-secondary" disabled={uploading}>
                ...
                {form.image_url && <img src={form.image_url} alt="Preview" className="mt-2 h-16 w-16 rounded-full object-cover" />}
              </div>
              <div>
              ...
              <div>
                ...
                <input value={form.video_url} onChange={(e) => setForm({ ...form, video_url: e.target.value })} className="admin-input" />
              </div>
              <div>
                <label className="admin-label">Influencer Video URL</label>
                <input value={form.influencer_video_url} onChange={(e) => setForm({ ...form, influencer_video_url: e.target.value })} className="admin-input" />
              </div>
            </div>
            <div>
